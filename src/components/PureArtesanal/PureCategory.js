...
export default class PureCategory extends Component {
  ...
  DeleteCategory(e) {
    ...
    console.log(e)
    fetch("https://artisanbackend.herokuapp.com/api/deleteCategory",
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json"
  ...
  async AddCategory() {
    ...
    await fetch("https://artisanbackend.herokuapp.com/api/addCategory",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
  ...
  async fetchIconDataType() {
    ...
    const response = await fetch("https://artisanbackend.herokuapp.com/api/getIcons" + iconname);
              ...
              {this.state.loading === false && this.state.Categories.length > 0 && this.state.Categories.map((categori, index) => {
                return (
                  ...
                  <ul className="collapse" id={"nw"+ index} style={{ "list-style": "none" ,  "fontFamily" :"Comic Sans MS" ,"backgroundColor" : "white" , "color" : "black"  }}>
                    ...
                    <li>  <button><li onClick={() => { this.setState({ adSubCat: true, addCat: false, categoryId: categori._id, categoryName: categori.name }) }} className=" bg-light"> <div className="col-md-1"></div> <i className="fa fa-plus" aria-hidden="true"></i> Add Sub Category</li></button> </li>
                    </div>
